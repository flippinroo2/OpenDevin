...
from src.logger import route_logger
...
def app_setup():
    ...
    logger.info("Creating directory for PDFs...")
    pdfs_dir = config.get_pdfs_dir()
    os.makedirs(pdfs_dir, exist_ok=True)

    logger.info("Creating directory for workspaces...")
    projects_dir = config.get_projects_dir()
    os.makedirs(projects_dir, exist_ok=True)

    logger.info("Creating directory for logs...")
...
app = Flask(__name__)
app.debug = config.is_debug_mode()  # Turning on debug mode for Flask
CORS(app)
...
os.environ["TOKENIZERS_PARALLELISM"] = "false"  # TODO: Figure out what this does
...
if __name__ == "__main__":
    logger.info("Code Partner is up and running!")
    socketio.run(
        app,
